...
// Response helper
const sendResponse = (res: Response, statusCode: number, data: { success: boolean; message?: string; data?: any; error?: string }): Response => {
...
export const getAllStores = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (province && province !== 'Semua Provinsi') {
            ...
            params.push(province as string)
        }
        ...
        if (store_type) {
            ...
            params.push(parseInt(store_type as string))
        }
        ...
        const transformedStores = (rows as any[]).map((store: any) => ({
            id: store._id,
            name: store.name,
            slug: store.slug,
...
export const getStoreReviews = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const [storeRows] = await connection.execute(
            'SELECT _id FROM affiliate_stores WHERE slug = ? AND is_active = 1',
            [slug]
        )
...
export const createStoreItem = async (req: AuthRequest, res: Response) => {
    ...
    try {
        ...
        const [rows] = await connection.execute(
            'SELECT _id FROM affiliate_stores WHERE _id = ? AND owner_id = ?',
            [storeId, userId]
        )
...
export const updateStoreItem = async (req: AuthRequest, res: Response) => {
    ...
    try {
        ...
        const [rows] = await connection.execute(
            `SELECT i._id FROM store_items i
             INNER JOIN affiliate_stores s ON i.store_id = s._id
             WHERE i._id = ? AND s._id = ? AND s.owner_id = ?`,
        ...
        if (fields.length > 0) {
            params.push(itemId)
            await connection.execute(
                `UPDATE store_items SET ${fields.join(', ')} WHERE _id = ?`,
                params
            )
        }
...
export const updateStore = async (req: AuthRequest, res: Response) => {
    ...
    try {
        ...
        await connection.execute(
            `UPDATE affiliate_stores SET ${fields.join(', ')} WHERE _id = ?`,
            params
        )
...
// Get store types
export const getStoreTypes = async (req: Request, res: Response) => {
    const connection = await pool.getConnection()
...
export const createStoreReview = async (req: AuthRequest, res: Response) => {
    ...
    try {
        ...
        const [storeRows] = await connection.execute(
            'SELECT _id, owner_id FROM affiliate_stores WHERE slug = ? AND is_active = 1',
            [slug]
        )
        ...
        const [existingRows] = await connection.execute(
            'SELECT _id FROM store_reviews WHERE store_id = ? AND user_id = ? AND is_active = 1',
            [storeId, userId]
        )

        const existingReviews = existingRows as any[]
        if (existingReviews.length > 0) {
            const reviewId = existingReviews[0]._id
            await connection.execute(
                'UPDATE store_reviews SET rating = ?, review_text = ?, updated_at = CURRENT_TIMESTAMP WHERE _id = ?',
                [ratingValue, review_text || null, reviewId]
            )
        } else {
            await connection.execute(
                'INSERT INTO store_reviews (store_id, user_id, rating, review_text) VALUES (?, ?, ?, ?)',
                [storeId, userId, ratingValue, review_text || null]
            )
        }

        const [aggregateRows] = await connection.execute(
            'SELECT COUNT(*) as count, AVG(rating) as avgRating FROM store_reviews WHERE store_id = ? AND is_active = 1',
            [storeId]
        )

        const agg = (aggregateRows as any[])[0]
        const reviewCount = Number(agg.count || 0)
        const averageRating = Number(agg.avgRating || 0).toFixed(2)

        await connection.execute(
            'UPDATE affiliate_stores SET rating = ?, review_count = ? WHERE _id = ?',
            [averageRating, reviewCount, storeId]
        )
