...
function StoreCard({ store }: { store: Store }) {
    return (
        <Link to={`/affiliate/${store.slug}`} className="block">
            <div className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow group">
                <div className="aspect-video overflow-hidden">
                    <img src={getImageUrl(store.image)} alt={store.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                </div>
                <div className="p-6">
                    ...
                    <div className="inline-flex items-center justify-center w-full px-4 py-3 bg-green-600 hover:bg-green-700 text-white font-medium rounded-xl transition-colors">
...
export default function Affiliate() {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                // Fetch provinces from AI API first
                const provRes = await fetch(`${AI_API}/provinces`)
                const provData = await provRes.json()
                const fetchedProvinces = provData.provinces || []
                setProvinces(fetchedProvinces)

                // If logged in, try to get user province
                if (token && user) {
                    try {
                        const API_URL = import.meta.env.VITE_API_URL || "http://localhost:3001"
                ...
                // Fetch stores
                const API_URL = import.meta.env.VITE_API_URL || "http://localhost:3001"
                const storesRes = await fetch(`${API_URL}/api/affiliate/stores`)
    ...
    const filteredStores = useMemo(() => {
        let result = stores
        if (searchQuery) {
            result = result.filter((store) => store.name.toLowerCase().includes(searchQuery.toLowerCase()))
        }
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if (!target.closest(".relative")) setIsFilterOpen(false)
        }
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pt-24 pb-16">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 mb-8 border border-gray-200 dark:border-gray-700">
                    <div className="flex flex-col md:flex-row gap-4">
                        <div className="flex-1 relative">
                            ...
                            <input type="text" placeholder="Cari nama store..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                        ...
                        <div className="relative">
                            <button onClick={() => setIsFilterOpen(!isFilterOpen)}
                                className="flex items-center justify-between w-full md:w-64 px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white">
                                <div className="flex items-center">
                            ...
                            {isFilterOpen && (
                                <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg z-10 max-h-60 overflow-y-auto">
                                    {["Semua Provinsi", ...provinces].map((province) => (
                                        <button key={province} onClick={() => { setSelectedProvince(province); setIsFilterOpen(false) }}
                                            className={`w-full text-left px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 ${
                                                selectedProvince === province ? "bg-green-50 dark:bg-green-900/20 text-green-600" : "text-gray-900 dark:text-white"
                                            }`}>
                                            {province}
                                        </button>
                                    ))}
                ...
                {error ? (
                    <div className="text-center py-12"><p className="text-red-500 text-lg">{error}</p></div>
                ) : (
                    <>
                        <p className="text-gray-500 dark:text-gray-400 mb-6">
                            Menampilkan {filteredStores.length} store {selectedProvince !== "Semua Provinsi" && <span className="text-green-600">di {selectedProvince}</span>}
                        </p>
                        {filteredStores.length > 0 ? (
                            ...
                            <div className="text-center py-12"><p className="text-gray-500 dark:text-gray-400 text-lg">Tidak ada store yang cocok dengan pencarian Anda</p></div>
                        )}
                    </>
                )}
            </div>
